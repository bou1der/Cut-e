...
const UserHeader = ({setModal,props}:{setModal:useState,props:profile}):JSX.Element =>{
    ...
    return (
        <div
            ...
            >
                <img className="w-full h-full bg-center bg-cover bg-no-repeat rounded-t-sm absolute background-profile transition duration-700 group-hover:brightness-75" style={{backgroundImage: props?.background ? 'url('+`${props.background}`+')': `url('https://storage.yandexcloud.net/test-cloud-boulder/46fbabda-b100-479f-8414-cd8cd91de369')`}} src={""} alt=""/>
                <div className="w-full h-full flex items-center z-10 relative justify-center">
                    <div className="relative group/avatar"
                         onClick={()=>{
                             setModal(true)
                         }}>
                        <img className="xl:w-40 xl:h-40 rounded-full transition group-hover/avatar:brightness-75" src={`${props.avatar || "https://storage.yandexcloud.net/test-cloud-boulder/ead6b826-0ebd-4812-9ec1-42a9c7abc076"}`}  alt=""/>
